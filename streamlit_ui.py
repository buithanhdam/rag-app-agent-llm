...
def find_image_urls(text: str):
    # Implement your image URL extraction logic here
    # For example, using regex as before
    pattern = r'(https?://[^\s]+(?:image\?[^\s]+))'
    matches = re.findall(pattern, text, re.DOTALL)
    # Clean up the matches: remove newlines and extra spaces
...
def send_query(text, api_key=None):
    ...
    resp = requests.post("http://127.0.0.1:8000/v1/complete", json=data, headers=headers , stream=True)
    return resp

# setup(knowledge_type)
    ...
    if query := st.chat_input():
        ...
        with st.chat_message("assistant"):
            ...
            for chunk in res.iter_content(
                ...
                # except:
                    # continue
            image_urls = find_image_urls(full_response)
            for url in image_urls:
                message_placeholder.image(url,width=300)
            message_placeholder.markdown(full_response)
            # Tìm URL hình ảnh trong response và hiển thị chúng
        # print(full_response)
